//simulate the LST(Least Slack Time First) schedule policy 
//for tasks scheduled and executed in Real Time System
#include <stdio.h>
#include <stdlib.h>
...
//get LST priority task
int lst_priority_task(int **jobs, int jobs_num, int current_time){
	int least_slack = INT_MAX, lst_task, all_done = 1, slack;
	
	for (int i =0;i < jobs_num;i++) {
		//slack = deadline − current_time − (time needed to complete the job)
		slack = jobs[i][2] - current_time - jobs[i][3];
		if( slack < least_slack && jobs[i][3] != 0) {
			least_slack = slack;
			lst_task = i+1;// pluse 1 since task number start from 1
		}
		if (jobs[i][3] != 0) {
			all_done = 0;
		}
...
void main() {
	...
	for (int i=0;i <= hp;i++) {
		...
		//get the LST priority of each task to assign task to be 
		//execuated for the current time slot i		
		sched_jobs[i] = lst_priority_task(jobs, jobs_num, i);
