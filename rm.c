//simulate the RM(Rate-Monotone) schedule policy 
//for tasks executed/scheduled in Real Time System
#include <stdio.h>
#include <stdlib.h>
...
//get rm priority task
int rm_priority_task(int **jobs, int jobs_num){
	int min_period = INT_MAX, rm_task, all_done = 1;
	
	for (int i =0;i < jobs_num;i++) {
		if(jobs[i][0] < min_period && jobs[i][3] != 0) {
			min_period = jobs[i][0];
			rm_task = i+1;// pluse 1 since task number start from 1
		}
		if (jobs[i][3] != 0) {
			all_done = 0;
		}
...
void main() {
	...
	for (int i=0;i < hp;i++) {
		...
		//get the rm priority of each task to assign task to be 
		//execuated for the current time slot i		
		sched_jobs[i] = rm_priority_task(jobs, jobs_num);
